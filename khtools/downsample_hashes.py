...
def downsample_siglist(
        siglist,
        downsample_num_hashes=None,
        downsample_scaled=None):
    if downsample_num_hashes is not None:
        siglist_downsampled = [s.minhash.downsample_n(
            downsample_num_hashes) for s in siglist]
    elif downsample_scaled is not None:
        siglist_downsampled = [s.minhash.downsample_scaled(
...
def compare_downsampled(siglist, num_hashes=None, scaled=None, names=None):
    siglist_downsampled = downsample_siglist(
        siglist,
        downsample_num_hashes=num_hashes,
    ...
    df = pd.DataFrame(
        values,
        index=names,
        columns=names).unstack().reset_index()
    df = df.rename(
        columns={
            'level_0': 'cell1',
            'level_1': 'cell2',
